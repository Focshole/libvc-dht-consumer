#ifndef CONSUMER_ARGS_CPP
#define CONSUMER_ARGS_CPP
...
args parseArgs(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, "h:c:p:P:s:", args_getopt_struct,
    ...
    switch (opt) {
    case 'c':
      params.priv_cert = optarg ? optarg : "";
      break;
    case 'p':
      params.priv_cert_passw = optarg ? optarg : "";
      break;
    case 'P':
      try {
        params.dht_port = atoi(optarg ? optarg : "");
      ...
      break;
    case 's':
      params.dht_bootstrap_socket = optarg ? optarg : "";
      break;
    case 'h':
      printUsage(argv[0]);
      params.help = true;
...
params parseAndHandleParams(int argc, char **argv) {
  ...
  if (!std::filesystem::exists(params.priv_cert.string() + ".pem") &&
    ...
    dht::crypto::Identity id = dht::crypto::generateEcIdentity("consumerNode");
